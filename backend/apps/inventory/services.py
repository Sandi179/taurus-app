...
class StockService:
    ...
    def get_closing_stock(item_id=None, location_id=None, as_of=None):
        ...
        opening_filter = ledger_filter & Q(ledger_entries__transaction_type='OPENING')

        result = qs.annotate(
            opening_qty=Coalesce(Sum('ledger_entries__quantity', filter=opening_filter), Decimal('0')),
            opening_value=Coalesce(Sum('ledger_entries__final_amount', filter=opening_filter), Decimal('0')),
            ...
            'opening_qty', 'opening_value', 'closing_qty', 'closing_value'
        ).order_by('name')
