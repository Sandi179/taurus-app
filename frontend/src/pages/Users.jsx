...
export default function UsersPage() {
  ...
  const deleteUser = async (u) => {
    if (!isAdmin) return toast.error('Admin access required.');
    if (u.id === currentUser?.id) return toast.error('You cannot delete yourself.');
    if (!window.confirm(`Are you sure you want to PERMANENTLY delete ${u.first_name}?`)) return;
    try {
          ...
          <div className="flex gap8">
            ...
            <button className="export-btn excel" onClick={async () => {
              try {
                ...
                const a = document.createElement('a'); a.href = url; a.download = 'users_report.xlsx'; a.click(); URL.revokeObjectURL(url);
            ...
            <button className="export-btn pdf" onClick={async () => {
              try {
                ...
                const a = document.createElement('a'); a.href = url; a.download = 'users_report.pdf'; a.click(); URL.revokeObjectURL(url);
                  ...
                  <td>
                    <div className="flex gap8">
                      ...
                      <button className={`btn btn-xs ${u.is_active ? 'btn-danger' : 'btn-success'}`}
                        ...
                        {u.id !== currentUser?.id && (
                          <button className="btn btn-danger btn-xs" onClick={() => deleteUser(u)}>
                            Del
                          </button>
                        )}
                      </div>
